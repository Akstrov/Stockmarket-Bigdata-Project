...
# Auto-refresh setup
# ----------------------------
tick = st_autorefresh(interval=10_000, key="auto_refresh")
...
with tabs[0]:
    ...
    if not stock_df.empty:
        ...
        with col3:
            total_posts = reddit_df["post_count"].sum() if not reddit_df.empty else 0
...
with tabs[1]:
    ...
    if not stock_df.empty:
        subtab1, subtab2, subtab3 = st.tabs(["Price Chart", "Volume", "Returns"])
        with subtab1:
            fig = go.Figure()
            fig.add_trace(
                go.Candlestick(
        ...
        with subtab3:
            if "daily_return" in stock_df.columns:
                colors = ["red" if x < 0 else "green" for x in stock_df["daily_return"]]
                fig = go.Figure()
                fig.add_trace(
                    go.Bar(
                        x=stock_df["trade_date"],
...
with tabs[3]:
    ...
    if stock_df.empty or reddit_df.empty:
        ...
        r_daily = (
            r.groupby("date")
            .agg({"post_count": "sum", "avg_score": "mean", "total_score": "sum"})
            .reset_index()
        )
        ...
        if len(merged) < 5:
            ...
            corr = merged[["return", "post_count", "avg_score", "total_score"]].corr()
            fig = px.imshow(
                corr,
                text_auto=".2f",
...
if has_predictions or not pred_df.empty:
    with tabs[4]:
        ...
        if pred_df.empty:
            ...
            direction_acc = (pred_df["actual_dir"] == pred_df["pred_dir"]).mean() * 100
            ...
            best = pred_df.nsmallest(5, "prediction_pct_error")[
                ["timestamp", "actual_price", "predicted_price", "prediction_pct_error"]
            ]
...
# st_autorefresh = st.sidebar.checkbox("Auto-refresh (10s)", value=False)
# if st_autorefresh:
